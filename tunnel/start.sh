...
ExecStart=/usr/bin/ssh -i /var/www/minecraft-server/configuration/minecraft-docker/ssh/key.pem -N -R ${MINECRAFT_SERVER_APP_PORT}:localhost:${MINECRAFT_SERVER_APP_PORT} -R ${MINECRAFT_SERVER_RCON_PORT}:localhost:${MINECRAFT_SERVER_RCON_PORT} ${SERVER_USER}@${SERVER_HOST}
Restart=always
RestartSec=10
